...
function startCards() {
  ...
  const arrayCards = Array.from(
    document.querySelector(".section-quize-cards__content").children
  );
...
const conteinerCategories = document.querySelector(
  ".categories__filters-list-container"
);
const inputLabel = conteinerCategories.querySelectorAll(".filter-item__label");
const btnRemoveClass =
  conteinerCategories.querySelectorAll(".filter-item__btn");
...
function getItemCard(className) {
  card.forEach((item) => {
    if (item.getAttribute("data-category") === className) {
      item.style.display = "flex";
      btnLoadMore.style.display = "none";
    } else {
      item.style.display = "none";
...
function mobileSlider() {
  ...
  if (
    ...
    sliderMobile.classList.contains("swiper-initialized")
  ) {
    mySwiper.destroy();
...
for (let i = 0; i < 50; i++) {
  sectionIntro.innerHTML +=
    '<svg class="intro__decor-1" \
   style=" animation-duration: ' +
    randomCircle(1, 20) +
    "s; opacity: " +
    randomCircle(0.1, 0.2) +
    "; \
   top: " +
    randomCircle(0, 100) +
    "%; right: " +
   ...
   width: " +
    ...
    "px; height: " +
    s +
    'px;" viewBox="0 0 22 22" fill="none"  xmlns="http://www.w3.org/2000/svg"> \
